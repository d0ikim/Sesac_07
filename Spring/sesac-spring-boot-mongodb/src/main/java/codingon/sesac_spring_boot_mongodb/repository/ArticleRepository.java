package codingon.sesac_spring_boot_mongodb.repository;
...
// MongoRepository: Spring Data mongoDB의 기본 Repository 인터페이스
...
//    텍스트 검색 결과를 점수 내림차순으로 조회
//    점수 기반 정렬: 검색 관련성이 높은 순서로 정렬
...
//    이름과 이메일로 AND 조건 검색 -> name = ? AND email = ?
    List<Article> findByNameAndEmail(String name, String email);

//    이름 또는 이메일로 OR 조건 검색 -> name = ? OR email = ?
...
//    이름에 특정 문자열이 포함된 문서 검색 -> name LIKE '%substring%'
...
//    ==== 커스텀 쿼리 ====
    @Query("{name: ?0}")    // 검색 조건: name이 첫번째 매개변수와 일치
    @Update("{$Set: {email: ?1}}")  // @Update: MongoDB의 업데이트 쿼리 작성, email필드를 두번째 매개변수로 설정 ($Set은 필드값을 설정하는 연산자)
    int updateEmailByName(String name, String email);   // 반환값 int? 일괄 업데이트된 문서(document) 개수
}